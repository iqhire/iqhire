<?php $page_id = "streamline-the-hiring-process"; ?>
<?php include($_SERVER['DOCUMENT_ROOT'].'/inc/header.php');?>

<div class="row pagehead">
	<div class="span12">
		<h1 class="cushycms" title="Heading">Simplify and Streamline the Hiring Process</h1>
	</div>
</div>
<div class="row">
	<div class="span8 cushycms" title="Content">
		<p class="intro">The Selectech® Applicant Tracking System is the foundation for a fully integrated hiring solution.  Integrated solutions save time and reduce errors, and this results in a lower cost of hire and shorter time to hire. The Selectech® system includes the following integration choices:</p>
		<ul>
			<li>A pre-built integration to the Selectech® Verification System which manages electronic I9s and is integrated with the federal E-Verify system.</li>
			<li>A pre-built integration to Inquirehire assessments services</li>
			<li>A pre-built integration to Inquirehire background screening services</li>
			<li>The Selectech® system can be integrated with an employer’s payroll or HRIS system</li>
		</ul>
	</div>
	<div class="span4">
		<ul class="nav nav-tabs nav-stacked">
			<?php include($_SERVER['DOCUMENT_ROOT'].'/inc/nav-solutions.php');?>
		</ul>
	</div>
</div>
<?php include($_SERVER['DOCUMENT_ROOT'].'/inc/footer.php');?>